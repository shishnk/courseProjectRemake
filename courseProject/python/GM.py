    ...
    for iter in range(len(func)):
        ...
        if f>=0:
            ...
            if f>=0:
                ...
                k = 0
        #print(a, i, j, k)
        res+=a * fac(i)*fac(j)*fac(k)/fac(i+j+k+2)
...
def makeM():
    ...
    #print(Integrate(str(expand(psi[0]*psi[0]))))
    for row in rs:
        #print(' '.join(["{0:.4f}".format(float(elem)) for elem in row]))
    ...
    for iter in range(len(func)):
        ...
        if f>=0:
            ...
            if f>=0:
                ...
                k = 0
        #print(a, i, j, k)
        res[flag]+=a * fac(i)*fac(j)*fac(k)/fac(i+j+k+2)
...
def makeG():
    ...
    for row in rs:
        for x in row:
            #print(i, j, ', '.join(["{0:.14f}".format(float(elem)) for elem in x]))
            print(i, j, ', '.join([str(elem) for elem in x]))
            j+=1
        i+=1
        j=0
